-- ============================================================
-- ULTRA-FIX: Skapa om create_invoice_on_checkout helt från scratch
-- ============================================================
-- Detta scriptet fokuserar BARA på den problematiska funktionen
-- ============================================================

-- 1. Radera funktionen helt
DROP FUNCTION IF EXISTS public.create_invoice_on_checkout() CASCADE;

-- 2. Skapa funktionen på nytt - GARANTERAT med rätt kolumnnamn
  ...
  IF NEW.status = 'checked_out' AND OLD.status != 'checked_out' THEN
    ...
    BEGIN
      ...
      LOOP
        ...
        -- OBS: Läser bs.quantity (från booking_services) men skriver till qty (i invoice_items)
        INSERT INTO invoice_items (invoice_id, description, qty, unit_price, amount)
        VALUES (
          v_invoice_id,
          v_description,
    ...
    LOOP
      ...
      IF v_service_price > 0 THEN
        INSERT INTO invoice_items (invoice_id, description, qty, unit_price, amount)
        VALUES (v_invoice_id, v_description, 1, v_service_price, v_service_price);
    ...
    IF NEW.discount_amount > 0 THEN
      INSERT INTO invoice_items (invoice_id, description, qty, unit_price, amount)
      VALUES (v_invoice_id, 'Rabatt', 1, -NEW.discount_amount, -NEW.discount_amount);
...
-- 4. VERIFIERING - räkna exakta förekomster
SELECT 
  'create_invoice_on_checkout' as function_name,
  (SELECT pg_get_functiondef(oid) FROM pg_proc WHERE proname = 'create_invoice_on_checkout') ~ 'INSERT INTO invoice_items[^)]*\(invoice_id, description, qty,' as insert_format_correct,
  CASE 
    WHEN (SELECT pg_get_functiondef(oid) FROM pg_proc WHERE proname = 'create_invoice_on_checkout') ~ 'INSERT INTO invoice_items[^)]*\(invoice_id, description, qty,'
    THEN '✅ KORREKT - Använder qty i INSERT'
    ELSE '❌ FEL - Använder quantity i INSERT'
  END as status;

-- Förväntat resultat: insert_format_correct = true, status = '✅ KORREKT'
